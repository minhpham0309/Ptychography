%% probe replacement (jianhua method)
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe_refine(ePIE_inputs,varargin)
...
filename = strcat('reconstruction_probe_replace_',filename,'_',job_ID);
...
%% GPU
if gpu == 1
    display('========ePIE probe refine reconstructing with GPU========')
    ...
    display('========ePIE probe refine reconstructing with CPU========')
end
cdp = class(diffpats);
