%% probe replacement (jianhua method modified by arjun)
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe_refine_2(ePIE_inputs,varargin)
...
filename = strcat('reconstruction_probe_replace_2_',filename,'_',job_ID);
...
for mm = 1:length(lambda)
    ...
    scoop_range(mm) = range(scoop_vec{mm});
...
%% GPU
if gpu == 1
    display('========ePIE probe refine(method 2) reconstructing with GPU========')
    ...
    display('========ePIE probe refine(method 2) reconstructing with CPU========')
end
cdp = class(diffpats);
