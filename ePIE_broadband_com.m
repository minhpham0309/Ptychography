%% center of mass alignment on probes
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_com(ePIE_inputs,varargin)
...
best_mode = ePIE_inputs(1).best_mode; %for aligning probes, corresponding to highest intensity
S = ePIE_inputs(1).S;
[~,job_ID] = system('echo $JOB_ID');
job_ID = job_ID(~isspace(job_ID));
nModes = length(pixel_size);
filename = strcat('reconstruction_com_',filename,'_',job_ID);
...
%% GPU
if gpu == 1
    display('========ePIE com reconstructing with GPU========')
    ...
    display('========ePIE com reconstructing with CPU========')
end
cdp = class(diffpats);
