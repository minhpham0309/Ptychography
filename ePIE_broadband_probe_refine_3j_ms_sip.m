...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe_refine_3j_ms_sip(ePIE_inputs,varargin)
%varargin = {beta_ap, beta_obj, modeSuppression}
optional_args = {1 0.7 0}; %default values for optional parameters
nva = length(varargin);
...
%% Get centre positions for cropping (should be a 2 by n vector)
    ...
    for aper = 1:nApert
        ...
        cropR_sub(aper,:,m) = cropC(aper,:,m);
        cropC_sub(aper,:,m) = cropR(aper,:,m);
    end
