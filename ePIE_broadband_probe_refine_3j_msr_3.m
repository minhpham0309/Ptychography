...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe_refine_3j_msr_3(ePIE_inputs,varargin)
...
filename = strcat('reconstruction_probe_replace_3j_msr_3_',filename,'_',job_ID);
...
if isfield(ePIE_inputs, 'miscNotes')
    ...
    miscNotes = 'shuffling pixels outside iteration loop';
end
...
%% Get centre positions for cropping (should be a 2 by n vector)
    ...
    for aper = 1:nApert
        ...
        cropR_sub(aper,px_shuffle,m) = cropR(aper,1:little_area,m);
        cropC_sub(aper,px_shuffle,m) = cropC(aper,1:little_area,m);
    end
