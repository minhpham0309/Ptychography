...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_globalS(ePIE_inputs,varargin)
...
filename = strcat('reconstruction_globalS_',filename,'_',job_ID);
...
%% Update the probe
  ...
  %% collect the weights
            S_j(m,aper) = sum(abs(aperture{m}(:)).^2);  
        end
%         if gpu == 1
