...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe_refine_3j_msr_7(ePIE_inputs,varargin)
...
filename = strcat('reconstruction_probe_replace_3j_msr_7_',filename,'_',job_ID);
...
if isfield(ePIE_inputs, 'rot_deg')
    rot_deg = ePIE_inputs(1).rot_deg;
else
    rot_deg = 90; %rotation of sub modes positions
end
if isfield(ePIE_inputs, 'miscNotes')
    miscNotes = ePIE_inputs.miscNotes;
else
    miscNotes = 'rotating sub mode positions';
end
...
%% Get centre positions for cropping (should be a 2 by n vector)
    [pixelPositions, rotatedPositions, bigx, bigy] = convert_to_pixel_positions_testing6(positions,pixel_size(m),little_area,rot_deg);
    centrey = round(pixelPositions(:,2));
    centrex = round(pixelPositions(:,1));
    centrey_sub = round(rotatedPositions(:,2));
    centrex_sub = round(rotatedPositions(:,1));
    centBig = round((bigx+1)/2);
    for aper = 1:nApert
        cropR(aper,:,m) = cropVec + centrey(aper);
        cropC(aper,:,m) = cropVec + centrex(aper);
        cropR_sub(aper,:,m) = cropVec + centrey_sub(aper);
        cropC_sub(aper,:,m) = cropVec + centrex_sub(aper);
    end
