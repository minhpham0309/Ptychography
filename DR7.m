...
freeze_aperture = Inf;
optional_args = {.9 0.1 0.0 1}; %default values for varargin parameters
nva = length(varargin);
optional_args(1:nva) = varargin;
[beta_obj, beta_ap, weight, probeNorm] = optional_args{:};
...
%% Update the object
        ...
        diff = Pu_old - Pu_new;
        alpha_obj = beta_obj*sqrt( max(1,iterations-t)/iterations );
        dt = alpha_obj/probe_max;
        u_temp = ((1-alpha_obj)*u_old + dt*Pu_new.*conj(aperture)) ./ ( 1-alpha_obj + dt*abs(aperture).^2 );
...
%% Update the probe
        if update_aperture == 1 
            ...
            %aperture = (aperture + ds*Pu_new.*conj(u_new)) ./ ( 1 + ds*abs(u_new).^2 );
        end
    ...
    if  mod(t,showim) == 0 && imout == 1;        
        ...
        fprintf('%d. Error = %f, scale = %.4f, dt = %.3f, ds = %.3f, weight = %f\n',t,errors(t),scale,dt, ds, weight);
