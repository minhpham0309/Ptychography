...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe2(ePIE_inputs,varargin)
...
for itt = 1:iterations
    ...
    for aper = randperm(nApert)
        ...
        if rand<0.1
            ...
            for m=1:nModes
                F_probe = fftshift(fft2(aperture{m})).*H_bk{m};
                %
                F_probe_pad = padarray(F_probe,  [pad_pre(m) pad_pre(m)],'pre');
                F_probe_pad = padarray(F_probe_pad, [pad_post(m) pad_post(m)],'post');
                probe_pad = ifft2(ifftshift(F_probe_pad));
                probe =  probe_pad(pad_pre(m)+1:end-pad_post(m),pad_pre(m)+1:end-pad_post(m));
                %}
                %{
            ...
            probe_avg = probe_sum./nModes;            
            for m=1:nModes
                %
                probe_pad = padarray(probe_avg, [pad_pre(m) pad_pre(m)],'pre');
                probe_pad = padarray(probe_pad, [pad_post(m) pad_post(m)],'post');
                F_probe_pad = fftshift(fft2(probe_pad));
                ...
                %}
                F_probe = F_probe./H_bk{m};
                probe = ifft2(ifftshift(F_probe));
