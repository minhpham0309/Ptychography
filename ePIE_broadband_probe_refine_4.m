%% testing fast method with only one fft/ifft in probe refinement loop
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe_refine_4(ePIE_inputs,varargin)
...
filename = strcat('reconstruction_probe_replace_4',filename,'_',job_ID);
...
for mm = 1:length(lambda)
    ...
    if mm ~= central_mode
        ...
        if lambda(mm)<lambda(central_mode) %higher energy
            H = padarray(H, [pad_pre(mm) pad_pre(mm)], 'pre');
            H = padarray(H, [pad_post(mm) pad_post(mm)],'post');
            H_fwd{mm} = my_ifft(H);
        elseif lambda(mm) > lambda(central_mode) %lower energy
            H_fwd{mm} = my_ifft(H(scoop_vec{mm},scoop_vec{mm}));
        end
        ...
        H_bk = exp(1i*k*-fresnel_dist).*exp(-1i*pi*lambda(mm)*-fresnel_dist*(u.^2+v.^2));
    end
end

%% GPU
if gpu == 1
    display('========ePIE probe refine(method 4) reconstructing with GPU========')
    ...
    display('========ePIE probe refine(method 4) reconstructing with CPU========')
end
cdp = class(diffpats);
...
%                     ,pixel_size(central_mode),-fresnel_dist,lambda(central_mode));
                ...
                if scoop_range(m) > little_area %higher energy than central mode
                    Fcentral_probe = my_fft(aperture{central_mode}).*H_bk;
                    ...
                    probe_rpl = my_ifft(Fprobe_replaced.*H_fwd{m});
...
%                     probe_rpl = ifftn((fftn(probe_rpl).*H_fwd{m}));
                elseif scoop_range(m) < little_area %lower energy than central mode
                    ...
                    probe_rpl(scoop_vec{m},scoop_vec{m}) = my_ifft(Fcentral_probe_cropped.*H_fwd{m});
