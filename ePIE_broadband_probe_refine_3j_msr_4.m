...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe_refine_3j_msr_4(ePIE_inputs,varargin)
...
filename = strcat('reconstruction_probe_replace_3j_msr_4_',filename,'_',job_ID);
...
if isfield(ePIE_inputs, 'miscNotes')
    ...
    miscNotes = 'shuffling pixels every iteration';
end
...
for itt = 1:iterations
    ...
    cropR_sub(:,px_shuffle,:) = cropR(:,1:little_area,:);
    cropC_sub(:,px_shuffle,:) = cropC(:,1:little_area,:);
