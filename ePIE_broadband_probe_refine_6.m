%% averaging all probes at lowest energy
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_broadband_probe_refine_6(ePIE_inputs,varargin)
...
filename = strcat('reconstruction_probe_replace_6',filename,'_',job_ID);
...
for mm = 1:length(lambda)
    pad_size = round(little_area/scaling_ratio(mm));
...
%% GPU
if gpu == 1
    display('========ePIE probe refine(method 6) reconstructing with GPU========')
    ...
    display('========ePIE probe refine(method 6) reconstructing with CPU========')
end
cdp = class(diffpats);
...
%         avgFpinh = zeros(little_area,little_area,nModes); 
        avg_pinh = zeros(little_area,little_area,nModes);
        for m = 1:length(lambda)
