...
function [big_obj,aperture,fourier_error,initial_obj,initial_aperture] = ePIE_DR_broadband(ePIE_inputs,varargin)
...
[beta_ap, beta_obj, modeSuppression, probe_norm] = optional_args{:};
rng('shuffle','twister');
...
%         display('applying loose support')
    ...
    if big_obj{m} == 0
        ...
        big_obj{m} = single(ones(bigx,bigy));
...
for itt = 1:iterations
    ...
    for aper = randperm(nApert)
        ...
        %% collect magnitude
        for m = 1:length(lambda)
            u_i{m} = big_obj{m}(cropR(aper,:,m), cropC(aper,:,m));
            v_i{m} = V{m}(cropR(aper,:,m), cropC(aper,:,m));            

            %weight = sqrt(S(m)) ./ ((sum(abs(aperture{m}(:)).^2)))^0.5;
            Pv{m} = v_i{m}.*aperture{m};
            z_temp{m} = fft2(Pv{m});
            collected_mag = collected_mag + abs(z_temp{m}).^2;
        end
        ...
        for m = 1:length(lambda)                
            % update object
            z_i = scale.*z_temp{m};            
            Pu_new = ifft2(2*z_i-z_temp{m});
            diff = Pu_new - Pv{m};
            
            object_max = max(abs(u_i{m}(:)));
            probe_max = max(abs(aperture{m}(:)));
            
            dt =  beta_obj./probe_max^2;
            u_temp = u_i{m} + dt.*conj(aperture{m}).*diff;
            %u_temp = ((1-beta_obj)*u_i{m} + dt*Pu_new.*conj(aperture{m})) ./ ( 1-beta_obj + dt*abs(aperture{m}).^2 );
            
            u_new = u_temp;           
            if realness == 1, u_new = real(u_new);  end
            if strongPosi == 1, u_new = max(0,real(u_new)); end
            if do_posi == 1 && strongPosi == 0
                u_new(u_new < 0) = u_i{m}(u_new < 0) - beta_pos.*u_new(u_new < 0);
            end
            
            v_i{m} = 0.5*v_i{m} + u_new - 0.5*u_temp;
            
            V{m}(cropR(aper,:,m), cropC(aper,:,m)) = v_i{m};
            ...
            if itt > update_aperture_itt && updateAp == 1
                if modeSuppression == 0 || mod(m,3) ~= 0
                    ...
                    aperture{m} = aperture{m} +ds*conj(u_i{m}).*(diff);
                    %aperture{m} = ((1-beta_ap)*aperture{m} + ds*Pu_new.*conj(u_i{m})) ./ ( (1-beta_ap) + ds*abs(u_i{m}).^2 );
                end
            ...
            fourier_error(itt,aper) = sum(abs(current_dp(goodInds)...
                - collected_mag(goodInds))) ./ sum(current_dp(goodInds));

    end
    %% plot result
    for m=1:length(lambda)
        figure(m);
        subplot(1,2,1);imagesc(abs(big_obj{m})); axis image; colormap jet;
        subplot(1,2,2);imagesc(abs(aperture{m})); axis image; colormap jet;
        drawnow;
    end
